...
export default function RecentProducts() {
  let { addProductToCart, setNumbersOfCartItems, numbersOfCartItems } =
  ...
  async function addToCart(id) {
    // 1. هنا مش هنحط Icon يدوية عشان التوست يعرف يبدلها لما يخلص
    const toastId = toast.loading("Adding to your cart...", {
      style: {
        minWidth: "300px",
        padding: "16px",
        fontSize: "22px", // تكبير الخط هنا هيكبر الدائرة التلقائية
      },
    });
    ...
    try {
      ...
      if (res?.data?.status === "success") {
        toast.success("Product added successfully! 🛒", {
          id: toastId, // دي اللي بتبدل الـ Loading بالـ Success
          duration: 3000,
          style: {
            minWidth: "300px",
            padding: "16px",
          ...
          // هنا بنأكد عليه يرجع يستخدم أيقونة الـ Success بتاعته
          icon: "✅",
        });
      } else {
        toast.error("Failed!", { id: toastId });
